...
2. Ask for the intended use: research, personal memory, project work, learning, writing, or team knowledge.
...
5. Add `status: seed` or `status: draft` until the human validates the structure.
...
2. Summarize the source in `sources/...` or append to `sources/source-log`.
...
7. Add contradictions under a heading like `## Tensions` or `## Contradictions`.
...
Use `tags: [needs-review]` for pages needing human attention regardless of status.

## Answer filing pattern

After answering a question from wiki context, file durable output when it adds value:
